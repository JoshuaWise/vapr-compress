...
module.exports = (lateHandler, encodeByDefault) => (req) => {
	let encoding;
	// Use Transfer-Encoding if it's supported by the client.
	const teHeader = req.headers.get('te');
	if (teHeader) {
		try { encoding = getTopEncoding(negotiated.transferEncodings(teHeader), transferEncodings); }
		catch (_) { return [400, 'Malformed TE Header']; }
		if (!encoding) return [501, 'No Acceptable Transfer-Encoding'];
		if (encoding !== 'identity') return lateHandler(req, 'transfer-encoding', encoding);
	}
	// Otherwise, use Content-Encoding if it's supported by the client.
	const acHeader = req.headers.get('accept-encoding');
	if (acHeader !== undefined) {
		try { encoding = getTopEncoding(negotiated.encodings(acHeader), contentEncodings); }
		catch (_) { return [400, 'Malformed Accept-Encoding Header']; }
		if (!encoding) return [406, 'No Acceptable Content-Encoding'];
		if (encoding !== 'identity') return lateHandler(req, 'content-encoding', encoding);
	} else {
		// If no Accept-Encoding header was provided, any encoding is valid.
		if (encodeByDefault) return lateHandler(req, 'content-encoding', 'gzip');
	}
